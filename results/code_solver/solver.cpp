	...
	for (auto& layer: global_grid.layers){ 
		...
		if ( layer.name=="neutrals" ){ } else { 
            ...
            // if more than one cell move
          	if ( SQRT(SQU(Dr)+SQU(Dz)) >= 1.0 ) {
          		waprintf ( ">> flying free path=%g too big",
													SQRT(SQU(Dr)+SQU(Dz)) );
							break;
          	}

						// relative position of particle in cell
          	if ( particle.oldpart==TOP_LEFT ) {
          		rP0 = rP - (rO+1);
          		zP0 = zP - zO;
          	} else if ( particle.oldpart==TOP_RIGHT ) {
          		rP0 = rP - (rO+1);
          		zP0 = zP - (zO+1);
          	} else if ( particle.oldpart==BOTTOM_LEFT ) {
          		rP0 = rP - rO;
          		zP0 = zP - zO;
          	} else if ( particle.oldpart==BOTTOM_RIGHT ) {
          		rP0 = rP - rO;
          		zP0 = zP - (zO+1);
          	}
...
/*** OLDPART == TOP LEFT *****************************************************************************/
						if ( pt.oldpart==TOP_LEFT ) {
							if ( pt.newpart==BOTTOM_LEFT ) {
								...
								if ( (rN==rO+1) && (zN==zO) ) {
									...
									four_face_current ( rO, zO, rpN, zpN, deltar, deltaz, qu, rP0, zP0 );
								} else if ( (rN==rO) && (zN==zO) ) {
									// seven face current 
									// old: top, left; p&m
									// topleft: bottom, right; m&p 
                  ...
                  seven_face_current ( rO. zO, rN, zN, qu, deltar, deltaz, rP0, zP0 );
