...
%
		Low-pressure capacitive rf plasma can be stabilzed mainly by collisionless heating in the sinusoidal rf modulated discharge sheaths, like it was proposed earlier in~\autoref{sec:langmuirlaw} and the following. Most theoretical models assume a `hard wall' approximation (HWA), where the electron are cosidered to collid elastically with the oscillating sheath edge. Heating power is then averaged by reverse and forward energy fluxes into and out of the sheath respectively. Thus giving an easy access to heating mechanisms of the proposed discharges, this model is very sensitive to \emph{ab initio} information about EEDF, sheath transit times and particle losses.~\cite{Gozadinos01b}.\\
		...
		Here one will consider the sheaths electric field as constant, $E=U\ix{sb}/s\ix{0}$, the bulks expansion $l$  and the sheaths thickness being modulated cosinusoidal. The equation of motion for an electron in the sheath is written below.
%
		\begin{align}
		...
		\end{align}
%
		The latter equation is introduced as dimensionless with the corresponding parameters: $\alpha=m\ix{e}\omega^{2}s\ix{0}^{2}/eU\ix{sb}$, $\beta=U\ix{rf}/U\ix{sb}$ and $\epsilon=s\ix{0}/l\,$. Integrating yields the velocity of an electron as it moves through the sheath $\mu(\tau)$.\\
...
%
%		
		There are different approaches on the heating in low-pressure, low-temperature rf plasma. 
